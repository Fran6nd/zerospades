...
namespace spades {
	...
	class StartupScreenGraphicsTab : spades::ui::UIElement, LabelAddable {
		...
		StartupScreenGraphicsTab(StartupScreenUI@ ui, Vector2 size) {
...

			{
...

				{
					...
					cplx.AddEditor(StartupScreenConfigSelectItemEditor(
						ui, StartupScreenConfig(ui, "r_water"), "0",
						_Tr("StartupScreen",
							"Water Shader|" "None:Water is rendered in the same way that normal blocks are done.")));
			...
			// Vulkan config view: identical to GL view but without Shader Effects (r_water is
			// locked to 0 in the Vulkan renderer and the block would mislead the user).
			{
				StartupScreenConfigView cfg(Manager);
