...
namespace spades {
	namespace draw {
		...
		void VulkanMapRenderer::CreatePipelines(VkRenderPass renderPass) {
			...
			pushConstantRange.size = sizeof(float) * 16 + sizeof(float) * 3; // mat4 + vec3
			...
			result = vkCreateGraphicsPipelines(vkDevice, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &basicPipeline);
