	...
	namespace draw {
		...
		void VulkanSSAOFilter::CreateRenderPass() {
			...
			// SSAO render pass (R8 output)
			{
				...
				if (vkCreateRenderPass(device->GetDevice(), &renderPassInfo, nullptr, &ssaoRenderPass) != VK_SUCCESS) {
		...
		void VulkanSSAOFilter::CreatePipelines() {
			...
			// Load programs
			ssaoProgram = renderer.RegisterProgram("Shaders/PostFilters/SSAO.vk.program");
			bilateralProgram = renderer.RegisterProgram("Shaders/PostFilters/BilateralFilter.vk.program");
