	...
	namespace draw {
		...
		void VulkanLensFlareFilter::CreateRenderPass() {
			// Scanner render pass
			{
				...
				if (vkCreateRenderPass(device->GetDevice(), &renderPassInfo, nullptr, &scannerRenderPass) != VK_SUCCESS) {
					SPRaise("Failed to create scanner render pass");
				}
			}
			...
			// Draw render pass (additive blending onto existing framebuffer)
			{
				...
				if (vkCreateRenderPass(device->GetDevice(), &renderPassInfo, nullptr, &drawRenderPass) != VK_SUCCESS) {
					SPRaise("Failed to create draw render pass");
				}
			}
			...
			// Base class render pass (for blur)
			{
				...
				if (vkCreateRenderPass(device->GetDevice(), &renderPassInfo, nullptr, &renderPass) != VK_SUCCESS) {
					SPRaise("Failed to create blur render pass");
				}
			}
		}
